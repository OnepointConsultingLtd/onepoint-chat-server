export default function FloatingChatButton({ click }: { click: () => void }) {
  return (
    <div className="flex justify-center fixed bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 w-full px-4 sm:px-0 z-40">
      <button
        onClick={click}
        className="group relative w-full sm:w-auto px-4 sm:px-6 py-3 sm:py-3.5 bg-gradient-to-r from-[color:var(--osca-accent)] to-[color:var(--osca-accent)] text-white font-semibold text-sm sm:text-base rounded-xl shadow-lg hover:shadow-[color:color-mix(in_srgb,var(--osca-accent)_25%,transparent)] active:scale-95 sm:hover:scale-105 transition-all duration-300 hover:from-[color:var(--osca-accent)] hover:to-[color:var(--osca-accent)] flex items-center justify-center gap-2 sm:gap-2.5"
      >
